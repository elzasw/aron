...
@Getter
@Setter
public class SwaggerDiscriminatorMappingAnnotationModel extends AnnotationModel {

    private ClassType schema;

    public SwaggerDiscriminatorMappingAnnotationModel(TypeModel type, ClassType schema, Map<String, Object> attributes, String[] views) {
    ...
    public static SwaggerDiscriminatorMappingAnnotationModel of(TypeModel typeModel, Map<String, Object> attributes, Map<MethodSymbol, Attribute> elementValues, String[] views) {
        ClassType schema = (ClassType) getElementValue("schema", elementValues, attribute -> (Attribute.Class) attribute).getValue();

        return new SwaggerDiscriminatorMappingAnnotationModel(typeModel, schema, attributes, views);
    }
    ...
    protected String toString(Map.Entry<String, Object> entry) {
        if (entry.getKey().equals("schema")) {
            ...
            if (viewName != null) { // replace only if view is present
                String className = schema.toString();
