...
public abstract class AuthoredService<
    ...
    public Result<LIST_PROJECTION> listMine(@Nullable Params params) {
        ...
        if (access != Access.NONE) {
            ...
            throw new ForbiddenOperation(NOT_ALLOWED);
        }
    }
    ...
    protected void preUpdateHook(@Nonnull ROOT object) {
        ...
        if (access == Access.TENANT) {
            ...
            eq(tenant, author, () -> new ForbiddenOperation(rootType, id, NOT_ALLOWED));

        }  else if (access == Access.SELF) {
            UserReference user = UserGenerator.generateValue();
            UserReference author = getAuthor(id);
            eq(user, author, () -> new ForbiddenOperation(rootType, id, NOT_ALLOWED));

        } else if (access == Access.NONE) {
            throw new ForbiddenOperation(rootType, id, NOT_ALLOWED);
        }
    }
