...
import static cz.inqool.eas.common.domain.index.dto.filter.FilterUtils.deNest;
    ...
    public Aggregation aggregationFactory() {
        if (multiBucket == null && bucket == null) {
            ...
            return FilterAggregation
                    .builder()
                    .name(this.getName())
                    .filter(deNest(new InFilter(this.getField(), multiBucket.getValues().toArray(String[]::new))))
            ...
            return FilterAggregation
                    .builder()
                    .name(this.getName())
                    .filter(deNest(new EqFilter(this.getField(), bucket.getValue())))
                    .aggregations(this.subAggregationsFactory())
                    .build();
        }
